...
class Stage extends Phaser.Scene {
    ...
    update(current_time, delta_time) {
        ...
        this.landscapeLayers.forEach((layer) => {
            console.log(`${layer.offsetIndex} - ${layer.y}`);
            if (layer.y > game.config.height) {
                layer.activeScroll = false;
            }
            if(isNaN(layer.y)) {
                layer.activeScroll = false;
                //layer.y = 0 - layer.layer.heightInPixels;
            }
            if (layer.activeScroll) {
                layer.y += mapScrollSpeed;
            }
        });
